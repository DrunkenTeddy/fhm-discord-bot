...
module.exports = async (teamInfo, orderBy = "pts") => {
  let sql = "";
  ...
  if (teamInfo) {
    ...
    if (orderBy === "pts") {
      sql += `
      WHERE trade_block.TeamID=${teamInfo.teamID} AND trade_block.Available=1 AND player_master.Retired=0
    ...
    sql = `
      ...
      LEFT JOIN player_master
      ON trade_block.PlayerID = player_master.PlayerID
      WHERE trade_block.Available=1 AND team_data.LeagueID=0 AND player_master.Retired=0
    `;

    if (orderBy === "pts") {
      sql += "ORDER BY PTS DESC, G DESC, GR DESC LIMIT 5;";
    } else if (orderBy === "name") {
      sql += "ORDER BY `Last Name` ASC LIMIT 5;";
    }
  }
